...
var addSocketsModule = function (context, session) {
	if (!_TEST_) {
		context.socket = io.getUserRoom(session.userId);
		context.sockets = io.getUsersSockets(); // Can do this.sockets.user('ID...').emit('XXX', data) or this.sockets.emit('YYY', data);
		return;
	}
